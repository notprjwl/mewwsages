            ...
            {users.map((user) => (
              <Stack key={user.id} direction='row' align="center" p={1} px={4} _hover={{ bg: "whiteAlpha.200" }} borderRadius={5} gap={3}>
                ...
                <Flex justify="space-between" width="100%" >
                  <Text alignContent="center" color="whiteAlpha.700"> {user.username}</Text>
                  <Button
                   size='sm' bg="brand.100" _hover={{ bg: "brand.100"}} fontFamily='myFont' fontWeight={200} onClick={() => {addParticipant(user)}} >
                    Select
                  </Button>
                </Flex>
              </Stack>
